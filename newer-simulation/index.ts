...
function satisfyConstraints(p1: Particle, p2: Particle, restDistance: number): void {
...
function animate() {
    ...
    const positionsAttribute = mesh.geometry.attributes.position as THREE.BufferAttribute;
    for (let i = 0; i < particles.length; i++) {
        positionsAttribute.setXYZ(i, particles[i].position.x, particles[i].position.y, particles[i].position.z);
    }
...
function updateMouseProjection() {
    ...
    if (intersects.length > 0) {
        ...
        interactionPlaneNormal.subVectors(camera.position, controls.target).normalize();
        interactionPlane.setFromNormalAndCoplanarPoint(interactionPlaneNormal,
            (selectedParticleIndex !== undefined ? particles[selectedParticleIndex].position : controls.target)
        );
...
function onPointerDown(event: PointerEvent) {
    if (event.button === 0) { // Left mouse button
        ...
        if (!mouse3D.equals(new THREE.Vector3(0,0,0)) || raycaster.intersectObject(mesh).length > 0) { // Check if mouse3D is valid
